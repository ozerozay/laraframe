...
import { forgeGetNginxConfig, forgeUpdateNginxConfig } from "@/lib/tauri";
...
export function NginxConfigWidget({ token, orgSlug, serverId, siteId }: Props) {
  ...
  const load = async (force = false) => {
    setLoading(true);
    if (force) invalidateCache(`site:${siteId}:nginx`);
    try {
      const cfg = await cachedFetch(`site:${siteId}:nginx`, () =>
        forgeGetNginxConfig(token, orgSlug, serverId, siteId)
      );
      setContent(cfg);
      setDraft(cfg);
      setLoaded(true);
  ...
  const save = async () => {
    setSaving(true);
    try {
      await forgeUpdateNginxConfig(token, orgSlug, serverId, siteId, draft);
      setContent(draft);
      setEditing(false);
      invalidateCache(`site:${siteId}:nginx`);
    ...
    <div className="flex flex-col h-full">
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-auto bg-zinc-100 dark:bg-zinc-950 px-2 py-3 font-mono text-sm leading-5">
          {content ? <LogViewer content={content} color="blue" /> : (
            <div className="flex h-full items-center justify-center text-muted-foreground/40">
              <FileCode2 className="h-5 w-5 mr-2" />
              <span className="text-xs">No Nginx config</span>
            </div>
          )}
        </div>
      )}
      <ConfirmDialog
        ...
        description="Incorrect Nginx configuration can take your site offline. Make sure the config is valid before saving."
        variant="warning"
        confirmText="Update"
        loading={saving}
